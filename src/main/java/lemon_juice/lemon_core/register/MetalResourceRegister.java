...
public class MetalResourceRegister {
    ...
    public void registerMetalGroup(String name, DataTable table){
        //Items
        LemonCoreItems.ITEMS.register(name + "_ingot", () -> new IngotItem(new Item.Properties(), table));
        LemonCoreItems.ITEMS.register(name + "_nugget", () -> new NuggetItem(new Item.Properties(), table));
        LemonCoreItems.ITEMS.register(name + "_dust", () -> new MetalDustItem(new Item.Properties(), table));

        //if(!table.getIsAlloy()) ModItems.ITEMS.register("raw_" + name, () -> new RawMetalItem(new Item.Properties(), table));

        //Blocks & BlockItems
        LemonCoreBlocks.registerBlock(name + "_block", () -> new MetalBlock(BlockBehaviour.Properties.copy(Blocks.IRON_BLOCK).requiresCorrectToolForDrops().strength(5.0F, 6.0F).sound(SoundType.METAL)), table);
